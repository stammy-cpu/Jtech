...
export default function GadgetDetailPage() {
  ...
  const { data: apiGadget, isLoading, error} = useQuery<Gadget>({
    queryKey: ["/api/gadgets", gadgetId],
  });

  const gadget = useMemo(() => {
    if (apiGadget) return apiGadget;
    const staticGadget = featuredGadgets.find(g => g.id === gadgetId);
    return staticGadget as Gadget | undefined;
  }, [apiGadget, gadgetId]);
